...
if($objLogin->IsLoggedInAs('owner','mainadmin') && Modules::IsModuleInstalled('rooms')){	
    ...
    if(FLATS_INSTEAD_OF_HOTELS){
        draw_title_bar(prepare_breadcrumbs(array(_FLATS_MANAGEMENT=>'',_FLATS=>'',_ROOMS_SETTINGS=>'',ucfirst($action)=>'')));
    }else{
        draw_title_bar(prepare_breadcrumbs(array(_HOTELS_MANAGEMENT=>'',_HOTELS_AND_ROOMS=>'',_ROOMS_SETTINGS=>'',ucfirst($action)=>'')));
    }
    echo '<br />';
